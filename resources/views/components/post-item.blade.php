<article class="flex bg-white flex-col shadow my-4 max-w-sm lg:max-w-2xl">
    <!-- Article Image -->
    <a class="hover:opacity-75 transition-opacity">
        @if ($post->thumbnail)
            <img src="{{ $post->thumbnail }}" class="w-full h-96">
        @else
    ...
    <div class="bg-white flex flex-col justify-start p-6">
        ...
        <p href="#" class="text-sm pb-3">
            By <a href="#" class="font-semibold hover:text-slate-800">{{ $post->user->name }}</a>, Published on <span class="font-semibold text-green-500">{{ $post->getFormatedDate() }}</span>
        </p>
